...
import { addStorage, updateStorage, deleteStorage, getStorages, uploadImage, deleteImage } from './firebase';  // Import des fonctions Firebase
...
function StorageManagement({ companyId }) {
  ...
  const handleDeleteStorage = async () => {
    if (selectedStorage) {
      try {
        // Si l'entrepôt a une image associée, la supprimer de Firebase Storage
        if (selectedStorage.imageUrl) {
          await deleteImage(selectedStorage.imageUrl);  // Appel à la fonction deleteImage
        }
